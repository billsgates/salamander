...
func (m *mysqlParticipationRepository) GetJoinedRooms(ctx context.Context, id int32) (res []domain.RoomItem, err error) {
	...
	if err := m.Conn.Table("participation").Select("service_providers.name as service_name, rooms.room_id, rooms.plan_name, rooms.room_status, participation.is_host, participation.payment_status").
